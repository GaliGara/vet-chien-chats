    ...
    <AnimatedSection className="bg-[#FFFDFB] px-4 py-18 sm:px-6 sm:py-20 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
          <SectionHeading
            eyebrow="Servicios"
            title="Cuidado veterinario boutique para cada etapa."
            description="Consulta médica, estética, prevención y adopciones con una experiencia clara, cálida y profesional."
          />
        ...
        <StaggerContainer className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {services.slice(0, 6).map((service, index) => {
            const Icon =
              veterinaryServiceIcons[index % veterinaryServiceIcons.length];
            return (
                ...
                <MotionCard>
                  <article className="h-full rounded-[1.75rem] border border-[#E8D6DE] bg-white p-6 shadow-[0_18px_50px_rgb(91_58_99/0.08)] transition-colors hover:border-[#DFA2BA]">
                    ...
                    <p className="mt-3 text-sm leading-7 text-[#7B6A80]">
                      {service.description ??
                        "Servicio preparado para una experiencia veterinaria clara y bien acompañada."}
                    </p>
                  </article>
                </MotionCard>
              </StaggerItem>
