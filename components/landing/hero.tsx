...
export function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto grid min-h-[calc(86svh-4rem)] w-full max-w-7xl items-center gap-10 px-4 py-10 sm:px-6 md:py-12 lg:grid-cols-[1.02fr_0.98fr] lg:px-8 lg:py-12">
        <motion.div
          initial={shouldReduceMotion ? false : { opacity: 0.98, y: 26 }}
        ...
        >
          <div className="mb-6 inline-flex max-w-full items-center gap-2 rounded-full border border-[#E8D6DE] bg-white/70 px-4 py-2 text-sm font-medium text-[#5B3A63] shadow-sm backdrop-blur">
            <Sparkles className="size-4 text-[#C89B8C]" />
            <span className="truncate">
              Cuidado boutique para perros y gatos
            </span>
          </div>
          <h1 className="max-w-[21rem] font-heading text-5xl leading-[1.02] text-[#2F2433] sm:max-w-4xl sm:text-6xl lg:text-7xl">
            Una experiencia cálida para cuidar, reservar y adoptar con amor.
          </h1>
          <p className="mt-6 max-w-[21rem] text-lg leading-8 text-[#7B6A80] sm:max-w-2xl sm:text-xl">
          ...
          <div className="mt-8 flex max-w-[21rem] flex-col gap-3 sm:max-w-none sm:flex-row sm:flex-wrap">
            <Button
              asChild
          ...
          <div className="mt-9 grid max-w-[21rem] grid-cols-1 gap-3 sm:max-w-2xl sm:grid-cols-3">
        ...
        <motion.div
          initial={shouldReduceMotion ? false : { opacity: 0.98, scale: 0.96 }}
          ...
          className="relative mx-auto w-full max-w-[21.5rem] sm:max-w-[560px] lg:justify-self-end"
        >
          <div className="relative aspect-[4/5] overflow-hidden rounded-[2.25rem] border border-white/80 bg-[linear-gradient(145deg,#F7F1FA_0%,#FFFDFB_46%,#FFF6F8_100%)] premium-shadow">
